...
export class TLIBrowser {
  ...
  public static async init(account: TwitterAccount) {
    ...
    const browser = await TLIBrowser.getBrowser(userDataDirectory, account)
    return new TLIBrowser(browser)
  }

  public async newPage() {
    return TLIBrowser.newPage(this.browser)
  }
  ...
  private static async login(page: Page, account: TwitterAccount) {
    const logger = Logger.configure('TLIBrowser.login')
